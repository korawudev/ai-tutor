...
from shared.models import Document, Chunk, ImportBatch, Base
from shared.database import get_db
from ..tools import fetch_document, parse_document, chunk_document, embed_chunks
...
async def check_duplicate(
    ...
    elif source_type == "file" and content:
...
async def process_document(
    ...
    try:
        ...
        # 存储知识块
        for i, (chunk, embedding) in enumerate(zip(chunks, embed_result.embeddings)):
            db_chunk = Chunk(
...
async def retry_document(
    ...
    try:
        if document.source_type == "url" and document.source_url:
            fetch_result = await fetch_document(document.source_url, method=document.scrape_method or "auto")
            if not fetch_result.success:
                document.status = "failed"
                document.error_message = fetch_result.error
                await db.commit()
