...
class TestGetPendingReviews:
    ...
    async def test_returns_pending(self, db, review_schedule_modules):
        ...
        schedule = ReviewSchedule(
            id=uuid4(), user_id=uuid4(), chunk_id=uuid4(),
            topic="T", source="feynman", next_review=now - timedelta(hours=1),
            interval_days=1, ease_factor=2.5, review_count=0,
            mastery_score=50.0, status="active", created_at=now
        )
...
class TestSubmitReviewResult:
    ...
    async def test_submits_result(self, db, review_schedule_modules):
        ...
        schedule = ReviewSchedule(
            id=uuid4(), user_id=uuid4(), chunk_id=uuid4(),
            topic="T", source="feynman", next_review=now,
        ...
        db.execute.return_value = mock_result
        db.refresh = AsyncMock()
        result = await review_schedule_modules.submit_review_result(db, schedule.id, schedule.user_id, "easy")
...
class TestGetReviewStats:
    ...
    async def test_returns_stats(self, db, review_schedule_modules):
        ...
        mastered = ReviewSchedule(
            id=uuid4(), user_id=uuid4(), chunk_id=uuid4(),
            topic="T", source="feynman", next_review=now + timedelta(days=10),
            interval_days=30, ease_factor=2.5, review_count=5,
            mastery_score=95.0, status="mastered", created_at=now
        )
        call_count = 0
