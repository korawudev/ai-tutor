...
async def dashboard(
    ...
    concept_count = (
        await db.execute(
            select(func.count(MasteryRecord.id)).where(MasteryRecord.user_id == user_id),
        )
    ).scalar() or 0
    mastered_count = (
        await db.execute(
    ...
    avg_mastery = (
        await db.execute(
            select(func.avg(MasteryRecord.mastery_score)).where(MasteryRecord.user_id == user_id),
        )
    ).scalar() or 0
    ...
    return {
        ...
        "today": {
            ...
            "quiz_avg_score": float(today_stats.quiz_avg_score)
            if today_stats and today_stats.quiz_avg_score is not None
            else 0,
            "feynman_sessions": today_stats.feynman_sessions if today_stats else 0,
            "feynman_avg_score": float(today_stats.feynman_avg_score)
            if today_stats and today_stats.feynman_avg_score is not None
            else 0,
    ...
    return [
        {
            ...
            "quiz_accuracy": float(r.quiz_accuracy),
            "feynman_score": float(r.feynman_score),
