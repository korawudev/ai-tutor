...
from shared.models import Thread, Run, RunCreate, RunResponse
...
重要：当用户已经解释得足够清楚，或者对话已经进行了5轮以上，你认为用户已经理解了核心概念时，在回复末尾加上 [END] 标记。例如："你已经解释得很清楚了！[END]"
...
async def _call_llm_stream(messages: list[dict], model: str = DEFAULT_MODEL):
    async with httpx.AsyncClient() as client:
        async with client.stream(
            ...
            json={"model": model, "messages": messages, "max_tokens": 2048, "temperature": 0.7, "stream": True},
            timeout=60.0,
        ) as resp:
...
async def _feynman_stream(thread_id: UUID, run_id: UUID, input_data: dict, db: AsyncSession):
    ...
    history_result = await db.execute(
        select(Run).where(Run.thread_id == thread_id, Run.status == "completed", Run.id != run_id)
    ...
    try:
        ...
        yield f"data: {json.dumps({'type': 'done', 'response': clean_response, 'should_end': should_end})}\n\n"
...
def _generate_simple_response(agent_type: str, input_data: dict) -> str:
    ...
    if agent_type == "feynman":
        return f"作为费曼学习助手，我来帮您理解这个概念。您提到的是：{user_input[:100]}... 请尝试用自己的话解释这个概念，我会评估您的理解程度。"
    elif agent_type == "rag":
        return f"根据知识库的内容，关于您提到的'{user_input[:50]}'，我为您整理了相关信息。这是一个很好的问题！"
    else:
