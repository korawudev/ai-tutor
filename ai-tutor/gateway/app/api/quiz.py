...
{{
    "questions": [
        {{"id": "q1", "type": "choice", "question": "题目", \
"options": {{"A": "a", "B": "b", "C": "c", "D": "d"}}, "answer": "A", \
"explanation": "简要说明为什么答案是A", "topic": "知识点", "points": 20}}
    ]
}}
            ...
            questions_data = [
                {
                    "id": "q1",
                    "type": "choice",
                    "question": f"关于 {data.topic or '通用知识'}，以下哪个正确？",
                    "options": {"A": "选项A", "B": "选项B", "C": "选项C", "D": "选项D"},
                    "answer": "A",
                    "explanation": (
                        f"关于 {data.topic or '通用知识'} 的正确表述是选项A，"
                        "其余选项不符合题干要求。"
                    ),
                    "topic": data.topic or "",
                    "points": 20,
        ...
        if len(questions_data) < data.question_count:
            try:
                more_questions = await generate_once(
                    f"\n当前题目数量不足 {data.question_count},"
                    f"请再补充 {data.question_count - len(questions_data)} 道与上面不同的题目。",
                )
...
async def _record_quiz_mastery(db, user_id, quiz, results, quiz_count=1):
    ...
    """
    per_topic = {}
    for q, r in zip(quiz.questions, results, strict=False):
...
async def submit_quiz(
        ...
        results.append(
            {
                ...
                or f"正确答案是 {correct_answer}，请对照题目回顾相关知识点。",
            },
        )
        if not is_correct:
    ...
    mastery_change = (
        10.0
        if (correct_count / max(quiz.total_questions, 1)) >= 0.9
        else 5.0
        if (correct_count / max(quiz.total_questions, 1)) >= 0.7
        else 0.0
    )
...
async def add_wrong_to_review(
    ...
    result = await db.execute(
        select(WrongQuestion).where(
            WrongQuestion.id.in_(data.wrong_ids),
            WrongQuestion.user_id == user_id,
        ),
    )
    wrongs = result.scalars().all()
...
def _resolve_review_answer(q: dict, correct_answer, explanation):
    ...
    detail = content if content else letter or ""
...
async def _get_knowledge_context(db, user_id, scope, topic, time_range):
    ...
    if scope == "time_range" and time_range:
        # time_range: {"days": N} 表示最近 N 天,
        # {"start": "YYYY-MM-DD", "end": "YYYY-MM-DD"} 表示起止日期
        now = datetime.utcnow()
    ...
    if scope == "topic" and topic:
        q_result = await db.execute(
            ...
            .where(
                ...
                Quiz.scope == "topic",
                Quiz.topic == topic,
                Quiz.status.in_(["pending", "completed"]),
            )
