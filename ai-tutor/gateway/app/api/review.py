...
from shared.models import (
    ReviewSchedule, ReviewLog, ReviewItem, ReviewListResponse,
    NormalReviewAnswer, NormalReviewSubmit, NormalReviewResponse,
    VerificationFailedSubmit, VerificationLog,
    FeynmanVerifySubmit, FeynmanVerifyResponse,
    SessionSnapshot, BatchCompleteSubmit, ReviewBatchStats
)
...
from review_agent.app.tools.schedule_manager import (
    get_pending_reviews as get_pending_reviews_svc,
    submit_normal_review as svc_submit_normal_review,
    submit_verification_failed as svc_submit_verification_failed,
    get_review_stats as get_review_stats_svc,
...
VERIFY_PROMPT = """你是一位费曼学习法验证专家。用户针对单个知识点提交了自己的解释，请与参考答案对比评分。
...
score 为 0-100 整数。strengths/weaknesses/suggestions 为字符串数组，每项一句话。correct_answer 必须给出简明准确的标准答案（综合参考答案与你的知识），即使用户未作答或答错也要给出。"""
...
async def _load_schedule_or_404(db: AsyncSession, schedule_id: UUID, user_id: UUID) -> ReviewSchedule:
    result = await db.execute(
        select(ReviewSchedule).where(ReviewSchedule.id == schedule_id, ReviewSchedule.user_id == user_id)
    )
...
def _to_review_item(s: ReviewSchedule) -> ReviewItem:
    return ReviewItem(
        schedule_id=s.id, chunk_id=s.chunk_id, topic=s.topic or "",
        answer=s.answer,
        mastery_score=float(s.mastery_score), next_review=s.next_review,
        interval_days=float(s.interval_days), review_count=s.review_count,
        status=s.status, source=s.source, reason=s.reason,
        is_mastered=bool(s.is_mastered), correct_streak=s.correct_streak
    )
...
async def get_pending_reviews(
    ...
    if source_type not in (None, "daily", "wrong_book", "manual"):
        raise HTTPException(status_code=422, detail="source_type must be one of: daily, wrong_book, manual")
    schedules = await get_pending_reviews_svc(db, user_id, limit, source_type, tag_filter)
    now = datetime.utcnow()
    
    overdue_q = select(ReviewSchedule).where(
        ReviewSchedule.user_id == user_id, ReviewSchedule.status == "active", ReviewSchedule.next_review < datetime.utcnow()
    )
...
async def submit_normal_review(
    ...
    result = await svc_submit_normal_review(db, data.schedule_id, user_id, data.answer, data.response_time_ms)
    if not result:
        raise HTTPException(status_code=404, detail="Review schedule not found")
    ...
    if schedule.topic:
        background_tasks.add_task(upsert_mastery, db, user_id, schedule.topic.strip(), chunk_id=schedule.chunk_id, review_score=float(schedule.mastery_score))
...
async def submit_verification_failed(
    ...
    result = await svc_submit_verification_failed(db, user_id, data.chunk_id, data.schedule_id, data.response_time_ms)
    if not result:
        raise HTTPException(status_code=404, detail="Review schedule not found")
...
async def feynman_verify(
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=f"LLM 评分失败: {str(e)[:200]}")
