...
def upgrade():
    ...
    op.add_column(
        "review_schedule",
        sa.Column("is_mastered", sa.Integer(), nullable=False, server_default="0"),
    )
    ...
    op.create_check_constraint(
        ...
        "review_logs",
        (
            "result IN ('easy','good','hard','forgot','mastered','vague',"
            "'forgotten','verification_failed')"
        ),
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "schedule_id",
            sa.dialects.postgresql.UUID(as_uuid=True),
            sa.ForeignKey("review_schedule.id"),
            nullable=True,
        ),
